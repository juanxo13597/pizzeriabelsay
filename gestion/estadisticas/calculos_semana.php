...
//////////    ESPACIO RESERVADO PARA LAS SENTENCIAS     ////////////
	...
	$sentencia_cliente_semana="SELECT SUM(precio_total) FROM `$tabla8` WHERE fecha>'$formato_semana' AND fecha<'$formato_fin_semana' AND id_usuario='$id_usuario'";   //Genera la sentencia para los visitantes unicos.

	$sentencia_mensajes_semana="SELECT COUNT(id_pedido) FROM `$tabla8` WHERE fecha>'$formato_semana' AND fecha<'$formato_fin_semana' AND mensaje='1'";   //Genera la sentencia para los visitantes unicos.
...
///////////   EJECUTAMOS LAS SENTENCIAS   ///////////////////////////////
	...
	$respuesta_clientes_semana=mysqli_query($conexion,$sentencia_cliente_semana);
	$gastos_cliente_semana=mysqli_fetch_row($respuesta_clientes_semana);
	$clientes_semana[]=number_format($gastos_cliente_semana[0],2,'.','');

	$respuesta_mensajes_semana=mysqli_query($conexion,$sentencia_mensajes_semana);
	$mensajes_semana=mysqli_fetch_row($respuesta_mensajes_semana);
	$sms_semana[]=$mensajes_semana[0];
